  ...
  return (
    <div ref={containerRef} className="bg-[#11100F] text-[#EBEAE8] font-sans relative">
      ...
      <section id="features" className="py-24 px-8 relative z-10 max-w-[1200px] mx-auto flex flex-col justify-center">
        ...
        <h2 className="text-4xl lg:text-5xl font-serif text-white mb-12 leading-tight max-w-3xl">
      ...
      <section className="py-32 px-8 relative z-10 max-w-[1200px] mx-auto flex flex-col justify-center">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
          <div className="lg:col-span-5 lg:sticky lg:top-32">
            ...
            <p className="text-lg text-[#A3A19E] mb-8 leading-relaxed">
            ...
            <ul className="flex flex-col gap-4">
              {[t('welcome.specimenPoint1'), t('welcome.specimenPoint2'), t('welcome.specimenPoint3'), t('welcome.specimenPoint4')].map((text, i) => (
                <li key={i} className="flex items-start gap-3 text-sm text-[#EBEAE8]">
      ...
      <section className="py-32 relative z-10 bg-[#181715] border-y border-[#2f2d2a]">
        <div className="max-w-[1200px] mx-auto px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
              <div className="text-[12px] tracking-[3px] text-[#A3A19E] uppercase font-bold mb-4">
                {t('welcome.libKicker')}
              </div>
              <h2 className="text-4xl lg:text-5xl font-serif text-white mb-6 leading-tight" dangerouslySetInnerHTML={{ __html: t('welcome.libTitle') }} />
              <p className="text-lg text-[#A3A19E] mb-10 leading-relaxed max-w-md">
                {t('welcome.libSub')}
              </p>
      ...
      <section className="min-h-[80vh] py-32 flex flex-col items-center justify-center relative z-10 bg-[#11100F] overflow-hidden cursor-default px-8">
        <motion.div style={{ scale: ctaScale }} className="relative z-10 w-full max-w-[1000px] bg-primary rounded-3xl p-8 lg:p-14 overflow-hidden shadow-2xl">
          ...
          <div className="relative grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="text-[12px] tracking-[3px] text-white/70 uppercase font-bold mb-4">
                {t('welcome.ctaKicker')}
              </div>
              <h2 className="text-4xl lg:text-5xl font-serif text-white mb-6 leading-tight" dangerouslySetInnerHTML={{ __html: t('welcome.ctaTitle2') }} />
              <p className="text-white/80 text-lg mb-8 max-w-md">
                {t('welcome.ctaSub2')}
              </p>
              <div className="flex flex-wrap gap-4">
                <button onClick={handleStart} className="bg-white text-primary px-8 py-4 rounded-xl text-base font-semibold hover:bg-gray-100 transition shadow-lg cursor-pointer flex items-center gap-2">
                  {t('welcome.beginFree')} <ArrowRight size={16} />
                </button>
                <button onClick={handleStart} className="border border-white/40 text-white px-8 py-4 rounded-xl text-base font-semibold hover:bg-white/10 transition cursor-pointer">
                  {t('welcome.seePro')}
                </button>
              </div>
            </div>
            <div className="bg-black/20 border border-white/20 rounded-2xl p-6 font-mono text-sm text-white shadow-inner">
...
const HeroPanel = ({ active, queueDuel, minutesToday, t }) => (
  <div className="bg-[#1C1B18]/80 backdrop-blur-md border border-[#2f2d2a] rounded-2xl p-6 font-mono text-xs flex flex-col gap-4 relative overflow-hidden shadow-2xl h-full">
    <div className="flex justify-between items-center">
      <span className="flex items-center gap-2 text-[#A3A19E] text-[10px] tracking-widest uppercase">
        <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse" /> {t('welcome.liveMissionControl')}
      </span>
      <span className="text-[#A3A19E]/50 text-[10px]">{t('welcome.version')}</span>
    </div>

    <div className="border-t border-[#2f2d2a] pt-4">
      <div className="text-[#A3A19E] text-[10px] tracking-widest mb-2 uppercase">{t('welcome.silentLibraryRooms')}</div>
      <div className="text-5xl lg:text-6xl text-white font-serif">{active.toLocaleString("en-IN")}</div>
      <div className="text-[11px] text-[#A3A19E] mt-2">{t('welcome.aspirantsDeepWork', { minutes: (minutesToday / 1000).toFixed(0) })}</div>
    </div>

    <div className="border-t border-[#2f2d2a] pt-3 grid grid-cols-2 gap-4">
      <div>
        <div className="text-[#A3A19E] text-[10px] tracking-widest uppercase">{t('welcome.duelQueue')}</div>
        <div className="text-2xl text-primary mt-1 font-serif">{queueDuel}</div>
      </div>
      <div>
        <div className="text-[#A3A19E] text-[10px] tracking-widest uppercase">{t('welcome.nationalTop')}</div>
        <div className="text-2xl text-white mt-1 font-serif">9,842</div>
      </div>
    </div>

    <div className="border-t border-[#2f2d2a] pt-3">
      <div className="text-[#A3A19E] text-[10px] tracking-widest mb-2 uppercase">{t('welcome.nextDrops')}</div>
    ...
    <div className="mt-auto border-t border-[#2f2d2a] pt-3 flex justify-between text-[#A3A19E] text-[10px] uppercase">
...
const DropRow = ({ t, l, tag, hot }) => (
  <div className="flex items-center gap-2 pb-1 border-b border-dashed border-[#2f2d2a]">
    <span className="text-[#A3A19E] w-10">{t}</span>
    <span className="flex-1 text-[#EBEAE8] truncate">{l}</span>
    <span className={`text-[9px] px-2 py-[2px] rounded-full tracking-widest border ${hot ? "border-primary text-primary" : "border-[#2f2d2a] text-[#A3A19E]"}`}>{tag}</span>
  </div>
);
...
const PYQSpecimen = ({ t }) => {
  ...
  return (
    <div className="border border-[#2f2d2a] rounded-2xl overflow-hidden bg-[#181715] shadow-2xl">
      <div className="bg-[#1C1B18] text-[#EBEAE8] px-5 py-3 flex justify-between font-mono text-[11px] tracking-widest border-b border-[#2f2d2a]">
        <span>{t('welcome.qHeader')}</span>
        <span className="hidden md:inline">{t('welcome.qDifficulty')}</span>
      </div>
      <div className="p-8">
        <p className="font-serif text-xl md:text-2xl leading-relaxed text-white m-0">
          {t('welcome.qText')}
        </p>
        <ol className="mt-5 pl-5 text-[#A3A19E] text-sm md:text-base leading-relaxed flex flex-col gap-2 list-none">
          <li>{t('welcome.qS1')}</li>
          <li>{t('welcome.qS2')}</li>
          <li>{t('welcome.qS3')}</li>
        </ol>
          ...
          {options.map((o) => {
            ...
            return (
              <button key={o.k} onClick={() => setChosen(o.k)} className={`text-left px-4 py-3 rounded-xl cursor-pointer flex items-center gap-3 text-sm transition-all border ${
                isCorrect ? "bg-[#2B7A4B]/20 border-[#2B7A4B] text-white" : 
                isWrong ? "bg-red-500/20 border-red-500 text-white" : 
                "bg-[#1C1B18] border-[#2f2d2a] text-[#A3A19E] hover:border-[#A3A19E]"
              }`}>
        ...
        {chosen && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-5 p-4 bg-[#1C1B18] rounded-xl border border-[#2f2d2a] text-sm text-[#A3A19E] leading-relaxed">
        ...
        <div className="mt-6 flex flex-col md:flex-row justify-between md:items-center font-mono text-[11px] text-[#716F6C] pt-4 border-t border-dashed border-[#2f2d2a] gap-2">
...
const LibraryViz = ({ active, t }) => {
  ...
  return (
    <div className="relative aspect-square bg-primary/5 border border-primary/20 rounded-3xl overflow-hidden p-5 flex-1 w-full max-w-md mx-auto">
      ...
      <div className="absolute top-5 left-5 font-mono text-[10px] text-[#A3A19E] tracking-widest uppercase">
        {t('welcome.libVizMap')}
      </div>
      <div className="absolute bottom-5 left-5 right-5 font-mono text-[10px] text-[#A3A19E] flex justify-between uppercase">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center text-white font-mono text-[10px] tracking-widest uppercase">
...
const DarkStat = ({ n, label, live }) => (
  <div>
    <div className="font-mono text-2xl md:text-3xl font-medium text-white flex items-center">
      {live && <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse mr-2" />}
      {n}
    </div>
    <div className="text-[11px] text-[#A3A19E] mt-1 font-mono tracking-widest uppercase">{label}</div>
  </div>
);

const FeatureCard = ({ col, tag, title, body, stat, icon, tone }) => {
  const toneMap = {
    rust: { bg: "bg-[#181715]", accent: "text-primary", border: "border-[#2f2d2a]", dotBg: "bg-[#2B2A27]" },
    indigo: { bg: "bg-[#181715]", accent: "text-indigo-400", border: "border-[#2f2d2a]", dotBg: "bg-[#2B2A27]" },
    moss: { bg: "bg-[#181715]", accent: "text-[#2B7A4B]", border: "border-[#2f2d2a]", dotBg: "bg-[#2B2A27]" },
    gold: { bg: "bg-[#181715]", accent: "text-[#BFA532]", border: "border-[#2f2d2a]", dotBg: "bg-[#2B2A27]" },
    ink: { bg: "bg-gradient-to-br from-[#2a2926] to-[#1f1e1b]", accent: "text-primary", border: "border-[#3A3935]", dotBg: "bg-white/10" }
  };
  const t = toneMap[tone] || toneMap.rust;
  
  return (
    <div className={`${col} ${t.bg} border ${t.border} p-6 lg:p-8 rounded-3xl flex flex-col gap-4 text-[#EBEAE8] min-h-[200px] shadow-lg hover:border-white/20 transition-all cursor-default`}>
      ...
      <h3 className="text-2xl font-serif text-white leading-tight">{title}</h3>
      <p className="text-sm text-[#A3A19E] leading-relaxed">{body}</p>
      <div className={`mt-auto pt-4 border-t border-dashed border-[#2f2d2a] font-mono text-[11px] text-[#716F6C] tracking-widest uppercase`}>
        {stat}
      </div>
    </div>
  );
...
const Row = ({ k, v }) => (
  <div className="flex justify-between border-b border-dashed border-[#2f2d2a] pb-1 text-xs">
    <span className="text-[#A3A19E]">{k}</span>
